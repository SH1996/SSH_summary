...
反转设置
=======
* 为了防止关联和级联出现双方的都能控制，会在更新时出现错误，所以在集合一方设置

级联操作
=======
* 不一定设置，如果希望在删除或更新等操作时能顺带的删除或更新这个对象关联的对象的话可以设置
...
<hibernate-mapping>
	<!-- 
	 ...
	 -->
    <class name="is.its.Student" table="student" >
    	...
    	<!-- 
    	 ...
    	 -->
    	<property name="sname" column="sname" length="30"/>
    	<!-- 多对多关系映射 -->
		<set name="courses" table="c_s" cascade="delete">
			<key column="sid"/>
			<many-to-many class="is.its.Course" column="cid"/>
		</set>
		<!-- 
...
<hibernate-mapping>
	<!-- 
		...
		如果需要采用继承映射，则class元素下还会增加<subclass.../>元素等用于定义子类。
	 -->
    <class name="is.its.Course" table="course" >
		...
		<set name="students"  table="s_c" inverse="true">
			<key column="cid"/>
			<many-to-many class="is.its.Student" column="sid"/>
		</set>		
		<!-- 
			组件映射：把多个属性打包在一起当一个属性使用，用来把类的粒度变小。
			<component name="属性，这里指对象">
